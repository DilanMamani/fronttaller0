...
const ADVERTENCIA_CONFIRMACION =
  'Para registrar una confirmación, la persona debe tener un bautismo previamente registrado en el sistema. Si creas una persona nueva, el registro fallará hasta que su bautismo sea cargado.';

export default function Paso2Confirmacion() {
  ...
  const handleConfirmar = () => {
    dispatch(clearError());
    setErrorPersona('');
    if (!confirmado) return setErrorPersona('Debes seleccionar la persona confirmada.');

    const rels = [{ rol_sacramento_id: 4, persona_id: confirmado.id_persona }];
  ...
  return (
    <div className="space-y-8">
      <SectionHeader icon="how_to_reg" title="Confirmación — Revisión de datos" />

      <Section title="Datos del documento">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Nombre del confirmado">
            <input type="text" value={campos.nombre} onChange={(e) => handleCampo('nombre', e.target.value)} className={ic()} />
          </Field>
      ...
      <Section title="Persona confirmada">
        <PersonaBuscador
          label="Buscar persona confirmada *"
          ...
          onSelect={(p) => { setConfirmado(p); setErrorPersona(''); }}
          onClear={() => setConfirmado(null)}
          personaSeleccionada={confirmado}
          advertencia={ADVERTENCIA_CONFIRMACION}
          error={!confirmado ? errorPersona : ''}
        />
      </Section>

      <Section
        title="Ministro / testigos"
        action={
      ...
      >
        {relaciones.length === 0 && <p className="text-xs text-gray-400">Sin relaciones agregadas.</p>}
